cat > webapp/src/components/AppAuthenticatedShell.tsx <<'EOF'
import { ArrowUpDown, Check, ChevronDown, Clock3, Cloud, Database, Folder as FolderIcon, Globe2, KeyRound, Lock, LogOut, MonitorSmartphone, Send as SendIcon, Settings as SettingsIcon, ShieldUser, SlidersHorizontal, Users } from 'lucide-preact';
...
export default function AppAuthenticatedShell(props: AppAuthenticatedShellProps) {
  ...
  const dataActive = props.location === '/backup' || props.location === '/settings/storage' || props.isImportRoute;
  ...
  const flatNav = (
    <>
      ...
      {isAdmin && renderSideLink('/settings/storage', props.location === '/settings/storage', <Database size={16} />, '存储库设置')}
    ...
    <>
      ...
      {renderNavGroup(
        ...
        <>
          ...
          {isAdmin && renderSubLink('/settings/storage', props.location === '/settings/storage', '存储库设置')}
